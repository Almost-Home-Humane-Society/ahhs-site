...
<li class="nav-section transition-all duration-400 ease-in-out">
  {#if expanded}
    <div class={classList}>
      <button class="nav-section-title cursor-pointer" onclick={() => {expanded = !expanded}}>
        {props.title}
  ...
  {:else}
    <button class="nav-item cursor-pointer py-2 px-3 text-sm rounded-md flex flex-row items-center justify-between" onclick={() => {expanded = !expanded}}>
      {props.title}
        <span class="iconify lucide--chevron-down size-5">
        </span>
    </button>
  {/if}
</li>
